    ...
    if ($row_number >= 1) {
        ...
        if ($result = mysql_query($sql, $con)) {
            ...
            if ($row_number >= 1) {
                ...
                $sql = "UPDATE events SET vote_yes=vote_yes+1 WHERE id =" . $event_id;
                if ($result = mysql_query($sql, $con)) {
                
                    $sql = "INSERT INTO `voter`(`event_id`, `user_id`, `vote_status`, `device_id`) VALUES (" . $event_id . ",'" . $user_id . "','yes','".$device_id."')";
                    if($result=mysql_query($sql,$con)){
